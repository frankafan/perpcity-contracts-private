...
library UintTWAP {
    ...
    /// @param price The price at the time of the new observation
    ...
    /// @param time The current block.timestamp
    /// @param target The timestamp at which the reserved observation should be for
    /// @param price The price at the time of the returned or simulated observation
    /// @param index The index of the observation that was most recently written to the observations array
    /// @param cardinality The number of populated elements in the oracle array
        ...
        returns (Observation memory beforeOrAt, Observation memory atOrAfter)
    {
        ...
        if (lte(time, beforeOrAt.blockTimestamp, target)) {
            if (beforeOrAt.blockTimestamp == target) {
                ...
                return (beforeOrAt, transform(beforeOrAt, target, price));
            }
        }
        ...
        returns (uint256 priceCumulative)
    {
        if (secondsAgo == 0) {
            Observation memory last = self[index];
            if (last.blockTimestamp != time) last = transform(last, time, price);
        ...
        (Observation memory beforeOrAt, Observation memory atOrAfter) =
            getSurroundingObservations(self, time, target, price, index, cardinality);
        ...
        returns (uint256[] memory priceCumulatives)
    {
        ...
        for (uint256 i = 0; i < secondsAgos.length; i++) {
            (priceCumulatives[i]) = observeSingle(self, time, secondsAgos[i], price, index, cardinality);
        }
    }
}
